...
# Flags
if not PARAMS.Flags.INTERACTIVE_PLOTING: 
    mpl.use('agg')
PLOT_SECDIFF = PARAMS.Flags.PLOT_SECDIFF # True
PLOT_SMOOTH_FIT = PARAMS.Flags.PLOT_SMOOTH_FIT # True
PLOT_ALL_GRID = PARAMS.Flags.PLOT_ALL_GRID # False
MAKE_PLOTS = PARAMS.Flags.MAKE_PLOTS # False
FILE_NOTICE =PARAMS. Flags.FILE_NOTICE # True
IMPOSE_TAU_HEII_TO_BE_WITHIN_THE_SEARCHING_RANGE = PARAMS.Flags.IMPOSE_TAU_HEII_TO_BE_WITHIN_THE_SEARCHING_RANGE # False

# Constants 
sun_age = PARAMS.Constants.Sun_age # 4.6e9 # yr
R_sun = PARAMS.Constants.Sun_radius # 6.95700e10 # cm

# Scales
time_scale = PARAMS.Scales.time # 1e9 (1 Gyr in yr)

# Others
min_delta_age = PARAMS.Constants.min_delta_age # 0.1 Gyr

# File names
file_notice_name = PARAMS.File_names.file_notice
...
frequencies_file = PARAMS.File_names.frequencies_file # 'frequencies.bin'
f = open(frequencies_file,'rb')
freq = pickle.load(f)
f.close()

# Read the history
h = mr.MesaData(PARAMS.Paths.MESA_history_file) # mr.MesaData('work/LOGS/history.data')
#model_numbers, profile_numbers = np.genfromtxt('work/LOGS/profiles.index', usecols=(0,2), unpack=True, skip_header=1, dtype=int)
model_numbers, profile_numbers = np.genfromtxt(PARAMS.Paths.MESA_index_profiles_file, usecols=(0,2), unpack=True, skip_header=1, dtype=int)
...
statistical_error_eigenfrequencies = PARAMS.Stat_err.Gaussian_sigma # 0.05 # micro-Hertz
n_statistical_error_realizations = PARAMS.Stat_err.number_of_realizations # 100
...
for profile,f in freq.items(): # f stands for frequencies
    ...
    for i_statistical_error_realization in range(n_statistical_error_realizations):
        ...
        profile_results = PARAMS.profile_results
        ...
        ell = np.unique(f['l'])
        # Save
        profile_results.l = ell # profile_results['l'] = ell
    
        # Save
        profile_results.age = age # profile_results['age'] = age
        ...
        # Save
        profile_results.secdiff = secdiff # profile_results['secdiff'] = secdiff
        profile_results.nu_secdiff =  nu_secdiff # profile_results['nu_secdiff'] = nu_secdiff
        profile_results.secdiff_no_statistical_errors = secdiff_no_statistical_errors # profile_results['secdiff_no_statistical_errors'] = secdiff_no_statistical_errors
        profile_results.nu_secdiff_no_statistical_errors = nu_secdiff_no_statistical_errors# profile_results['nu_secdiff_no_statistical_errors'] = nu_secdiff_no_statistical_errors
        ...
        while outliers:
            ...
            # Save
            profile_results.secdiff_filtered = secdiff_filtered # profile_results['secdiff_filtered'] = secdiff_filtered
            profile_results.nu_secdiff_filtered = nu_secdiff_filtered # profile_results['nu_secdiff_filtered'] = nu_secdiff_filtered
            ...
            profile_results.fit_smooth =  fit_smooth_dict # profile_results['fit_smooth'] = fit_smooth_dict 
    ...
    #       Limits or bounds for the fit    
            ...
            profile_results.fit_smooth_HeII = fit_smooth_HeII_dict # profile_results['fit_smooth_HeII'] = fit_smooth_HeII_dict 
    ...
    #        tau_BCZ_upper_lim = 1/(2*((1/1.05)*Delta_nu_peak))
        ...
        #    limits = np.concatenate([ [limits_lower], [limits_upper] ])
            ...
            profile_results.fit_smooth_HeII_BCZ = fit_smooth_HeII_BCZ_dict # profile_results['fit_smooth_HeII_BCZ'] = fit_smooth_HeII_BCZ_dict 
    ...
    #        tau_H_upper_lim = tau_H_init_guess + tau_H_init_guess*0.02
        ...
        #    cov_secdiff_smooth_HeII_BCZ_H[diag_ind] = diag
            ...
            profile_results.fit_smooth_HeII_BCZ_H = fit_smooth_HeII_BCZ_H_dict # profile_results['fit_smooth_HeII_BCZ_H'] = fit_smooth_HeII_BCZ_H_dict 
