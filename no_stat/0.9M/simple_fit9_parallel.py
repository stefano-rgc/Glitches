...
#    if not profile in ['profile1']: continue
                       
#    if profile == 'profile2': break
    
    # Variale where to save the results of each profile
    profile_results = dict()

    # Print which profile the loop is at
    print(' ' + profile)
    # Print it to a file (useful when using multiple procesor)
    if FILE_NOTICE:
        file_notice = open(file_notice_name,'a')
        file_notice.write(profile+'\n')
        file_notice.close()
...
#    # Read the corresponding profile (MESA output) from where the 
#    # frequencies were calculated from (with ADIPLS)
#    p = mr.MesaData('work/LOGS/'+profile+'.data')
#    
#    # Age of the profile in Gyr    
#    age = p.star_age / time_scale
    # Save
    ...
    for l in ell:
        ...
        secdiff[l] = f['secdiff'][ind]
        nu_secdiff[l] = f['nu'][ind]
...
#    for l in ell:
#        # Filter out the second differences that are greater than 5
#        ind = np.where( np.abs(secdiff[l]) < 5 )
#        # store in the dictionary
#        secdiff_filtered[l] = secdiff[l][ind]
#        nu_secdiff_filtered[l] = nu_secdiff[l][ind]
#        ind_filtered_out[l] = ind
    ...
    while outliers:
        ...
        # Initial guesses such that a0+a1x+a2x**2 is about 0 in the nu_little_osc
    #    c0_init_guess = 1/nu_high_osc
    #    c2_init_guess = 1/nu_high_osc**2
        tau_HeII_init_guess = 1/(2*Delta_nu_peak_He)
    #    print('tau_HeII_init_guess',tau_HeII_init_guess*1e6)
...
#        c0_lim = (0.5*c0_init_guess, c0_init_guess*2)
        c2_lim = (c2_lower_limit, c2_upper_limit)
#        c2_lim = (0.5*c2_init_guess, c2_init_guess*2)
    #    tau_HeII_lower_lim = 1/(2*(2*Delta_nu_peak_He))
    #    tau_HeII_upper_lim = 1/(2*(0.5*Delta_nu_peak_He))
#        Delta_nu_peak_He_uncertanty = 1.3 
#        over_estimation_Delta_nu_peak_He = Delta_nu_peak_He_uncertanty*Delta_nu_peak_He
#        under_estimation_Delta_nu_peak_He = (1/Delta_nu_peak_He_uncertanty)*Delta_nu_peak_He
    ...
    #    print('tau_HeII_lim',tau_HeII_lim[0]*1e6,tau_HeII_lim[1]*1e6)
        ...
        # INITIAL VALUES FOR THE FIT
        # Find the estimative wavelength
        indexes_peak = peakutils.indexes(residuals_smooth_HeII_dict['all_l_ordered'], min_dist=3, thres=0.1)
        nu_peak = all_nu_secdiff_filtered_sorted[indexes_peak] 
        Delta_nu_peak = np.diff(nu_peak)
    #    print('BCZ Delta_nu_peak MEAN', np.mean(Delta_nu_peak))
    #    print('BCZ Delta_nu_peak MEDIAN', np.median(Delta_nu_peak))
...
#        tau_BCZ_upper_lim = 1/(2*((1/1.05)*Delta_nu_peak))
    ...
    #    print('tau_BCZ_lim',tau_BCZ_lim[0]*1e6,tau_BCZ_lim[1]*1e6)
    #    print('')
...
#            fit_smooth_HeII_BCZ_H_grid_dict['secdiff_cov'] = cov_secdiff
    ...
    #        six_lists_list.append( grid_tau(i_tau_BCZ,i_tau_HeII,i_tau_H) )
        ...
        # ==========================================================================
        ### Let's check if any of the residuals in grid results is greater than the decided value    
        # ==========================================================================
...
# File where to store the results of the fit
savefile = open('fit_results.bin','wb')
...
if MAKE_PLOTS:
    ...
    for profile,fits in results.items(): # f stands for frequencies
